...
Das Spiel muss nicht vollständig neu entwickelt werden. Ersetzt werden Renderer und Terrain-Assetpipeline. Die bestehende Simulation bleibt kanonisch und ist über `spatial-prototype.html` live an denselben `geometry-sandbox-main.ts`-Renderpfad wie das Visual-Truth-Gate angebunden. Ein reiner Adapter übersetzt Koordinaten und Darstellungsdaten, aber keine Spielregeln. Der vorherige zweite Spatial-Renderer wurde entfernt. `geometry-sandbox.html` und `geometry-proof.html` bleiben eingefrorene Präsentations- und Diagnosenachweise, nicht Orte für neue Spielregeln. Details und Großszenen-QA stehen in [CANONICAL_GEOMETRY_INTEGRATION.md](CANONICAL_GEOMETRY_INTEGRATION.md).
